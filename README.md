...
- **Login to npm** (you need to be registered on npm as a user or an organization) using `npm login` in the **plugin folder** generated by the command `npm run build`, which has the same name as the one declared in the **package.json**.
